...
如果后台左侧只有“首页配置、主题配置、作品管理、导入导出”四个菜单，说明当前浏览器访问到的是旧版 `admin.html`。新版后台还应包含“维护检查”和“留言管理”，并且作品管理页支持点击或拖入图片上传。请关闭旧服务窗口，确认脚本窗口中的 `Project root` 是最新项目目录，再重新运行 `start-local.bat`。
